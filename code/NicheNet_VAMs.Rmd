...
# Filter for significant genes with padj < 0.01 and absolute log fold change in normalized counts (expression) > 2 and mean expression > 100
significant_genes <- res_df[res_df$padj < 0.01 & abs(res_df$log2FoldChange) > 2 & res_df$baseMean > 100, ] %>% distinct()
...
ggmaplot(
  ...
  ggtheme = theme_classic() + theme(plot.title = element_text(size = 20, hjust = 0.5, face = "bold"))
)
...
```{r}
# Here are codes to export a list of significant_genes and can be paste to GO term analysis https://geneontology.org/

# install.packages("clipr")
library('clipr')

# Convert the "gene" column to a character vector
genes <- as.character(expressed_ligands_VAMs)

# Combine the vector into a single string separated by newline characters
genes_export <- paste(genes, collapse = "\n")

# Copy to clipboard (Windows only)
write_clip(genes_export)
```
...
expressed_receptors_Adipocyte = intersect(receptors,expressed_genes_receiver_adipocyte$gene)
...
# > head(expressed_receptors_Adipocyte)
# [1] "Ddr1"  "Atrn"  "F11r"  "Mc2r"  "Mgrn1" "Lrp1" 
# > length(expressed_receptors_Adipocyte)
# [1] 376
...
p_ligand_target_network <- vis_ligand_target %>%
  make_heatmap_ggplot("Prioritized VAMs-ligands",
                      "potential adipogenesis genes adipocyte vs FRCs",
                      color = "purple",
                      legend_position = "top",
                      x_axis_position = "top",
                      legend_title = "Regulatory potential") +
  scale_fill_gradient2(low = "whitesmoke", high = "purple", breaks = c(0,0.1,0.3)) +
